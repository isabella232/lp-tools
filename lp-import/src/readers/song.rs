...
use lp::repositories::{SongNameRepository, SongRepository, SongUrlRepository};
...
fn names(ctx: &Context, root: &Value, song: &Song) -> Result<(), Error> {
    ...
    let values = root
        ...
        .ok_or_else(|| Error::Parse(String::from("expected song.names to be an array")))?;
...
fn contributions(ctx: &Context, array: &Value, song: &Song) -> Result<(), Error> {
    let values = array
        .as_array()
        .ok_or_else(|| Error::Parse(String::from("expected song.contributions to be an array")))?;
...
fn urls(ctx: &Context, array: &Value, song: &Song) -> Result<(), Error> {
    ...
    let values = array
        .as_array()
        .ok_or_else(|| Error::Parse(String::from("expected song.urls to be an array")))?;
