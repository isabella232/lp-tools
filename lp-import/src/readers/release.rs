...
pub fn create(ctx: &Context, root: &Value, album: &Album) -> Result<(Release, Vec<Medium>), Error> {
    ...
    let media = root
        .get("media")
        .ok_or_else(|| Error::Parse(String::from("expected release.media to exist")))
        .and_then(|array| media(ctx, &release, array))?;
...
fn new(ctx: &Context, root: &Value, album: &Album) -> Result<Release, Error> {
    let released_on = root
        .get("released-on")
        .and_then(Value::as_str)
        .ok_or_else(|| Error::Parse(String::from("expected release.released-on to be a string")))
        .and_then(|s| {
            NaiveDate::parse_from_str(s, "%Y-%m-%d")
                .map_err(|_| Error::Parse(format!("invalid release.released-on ({})", s)))
        })?;
...
fn urls(ctx: &Context, array: &Value, release: &Release) -> Result<(), Error> {
    ...
    let values = array
        .as_array()
        .ok_or_else(|| Error::Parse(String::from("expected release.urls to be an array")))?;
